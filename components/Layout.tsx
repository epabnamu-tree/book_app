...
import { Menu, X, BookOpen, Download, Users, Mail, UserCog, Facebook, PenTool } from 'lucide-react';
...
const Layout: React.FC<LayoutProps> = ({ children }) => {
  ...
  const navLinks = [
    ...
    { path: '/articles', label: '글 나누기', icon: <PenTool size={18} /> },
    { path: '/discussion', label: '수다 떨기', icon: <Users size={18} /> },
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            ...
            <div className="md:hidden">
              <button onClick={() => setIsMenuOpen(!isMenuOpen)} className="text-gray-500 hover:text-primary focus:outline-none">
              ...
              {navLinks.map((link) => (
                <Link
                  ...
                  className={`flex items-center gap-3 px-3 py-3 rounded-md text-base font-medium ${
                    isActive(link.path) ? 'bg-gray-50 text-secondary' : 'text-gray-600 hover:bg-gray-50 hover:text-primary'
                  }`}
                >
                  {link.icon}
                  {link.label}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-12 mb-12 border-b border-white/10 pb-12">
            <div className="md:col-span-2 flex flex-col items-start justify-center">
              <img src="https://i.imgur.com/qNKF9V9.png" alt="연구그룹 이팝나무" className="h-16 w-auto mb-4 opacity-90 hover:opacity-100 transition-opacity" />
            ...
            <div>
              ...
              <ul className="space-y-3 text-sm opacity-80">
                ...
                <li><Link to="/articles" className="hover:text-secondary transition-colors">글 나누기</Link></li>
                <li><Link to="/discussion" className="hover:text-secondary transition-colors">수다 떨기</Link></li>
            ...
            <div>
              ...
              <div className="flex space-x-4">
                <a href="https://www.facebook.com/epabnamu.org" target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full bg-white/10 flex items-center justify-center hover:bg-[#1877F2] hover:text-white transition-all duration-300 group" aria-label="Facebook">
          ...
          <div className="flex flex-col md:flex-row justify-between items-center gap-4 text-xs opacity-60 font-light">
            <p>© 2025 Research Group Epabnamu. All rights reserved.</p>
            <Link to="/admin" className="flex items-center gap-1 hover:text-secondary transition-colors"><UserCog size={12} /> 관리자 페이지</Link>
          </div>
        </div>
      </footer>
    </div>
